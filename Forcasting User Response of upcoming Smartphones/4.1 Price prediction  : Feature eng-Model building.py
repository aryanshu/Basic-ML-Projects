...
def XGBoost_hyperparameters_tuning(x_train,y_train,x_test,y_test,n_estimators=[100,200,300,400], max_depth=[ 40 , 60 ,70 , 80 ,90 ,100 ,110 ,200], eval_metric=['mae'] ):
	params={
		"learning_rate"    :[0.01],
		"max_depth"        :max_depth,
		"n_estimators"    :n_estimators,
	...
	return [y_predict_test,abs(y_predict_test-y_test).mean()]
...
if __name__ == '__main__':
	...
	print("XGB model with hyperparameters tuning : mean absolute error"+XGBoost_hyperparameters_tuning(x_train,y_train,x_test,y_test,n_estimators=[100,200,300,400], max_depth=[ 40 , 60 ,70 , 80 ,90 ,100 ,110 ,200], eval_metric=['mae'] )[1])

	# model with wich you want final result
	# change x_test and y_test to get result on test dataset. 
	y_predict = XGBoost_hyperparameters_tuning(x_train,y_train,x_test,y_test,n_estimators=[100,200,300,400], max_depth=[ 40 , 60 ,70 , 80 ,90 ,100 ,110 ,200], eval_metric=['mae'] )
	...
	print("Error in price "+abs(final_df['price']-final_df['predict']).mean())
